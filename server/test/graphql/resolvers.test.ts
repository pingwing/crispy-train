...
test('Query.inventoryItems maps all sort fields and defaults direction to ASC; empty filter becomes undefined', async () => {
  ...
  for (const c of cases) {
    ...
    const lastArgs = listInventoryItems.calls.at(-1)?.[0];
    assert.equal(lastArgs.filter, undefined);
    assert.deepEqual(lastArgs.sort, { field: c.repo, direction: 'ASC' });
  }
});

test('Query.storeInventorySummary delegates to inventoryService.getStoreInventorySummary', async () => {
  const store = new Store('s1', 'Store 1', undefined, d, d);
  const summary = {
    store,
    totalSkus: 2,
  ...
  const getStoreInventorySummary = createAsyncSpy(async (storeId: string) => {
...
test('Query.inventoryItems omits filter/sort/page/pageSize when not provided', async () => {
  const listInventoryItems = createAsyncSpy(async (args: any) => {
    assert.deepEqual(args, {
      filter: undefined,
      sort: undefined,
  ...
  await asResolverFn(resolvers.Query!.inventoryItems)({}, {}, ctx, null as any);
...
test('Mutation.updateStore maps unknown errors to GraphQLError INTERNAL', async () => {
...
test('Mutation.createProduct delegates to inventoryService.createProduct', async () => {
...
test('Mutation.createProduct maps ValidationError to GraphQLError BAD_USER_INPUT', async () => {
...
test('Mutation.upsertInventoryItem delegates to inventoryService.upsertInventoryItem', async () => {
  const store = new Store('s1', 'Store 1', undefined, d, d);
  const product = new Product('p1', 'Prod 1', 'Cat 1', d, d);
  ...
  const result = await asResolverFn(resolvers.Mutation!.upsertInventoryItem)(
    {},
    { input: { storeId: 's1', productId: 'p1', price: '12.50', quantity: 3 } },
    ctx,
    null as any,
  );
...
test('Mutation.upsertInventoryItem maps NotFoundError to GraphQLError NOT_FOUND', async () => {
  ...
  await assert.rejects(
    () =>
      asResolverFn(resolvers.Mutation!.upsertInventoryItem)(
        {},
        {
          input: { storeId: 's1', productId: 'p1', price: '1.00', quantity: 1 },
        },
        ctx,
        null as any,
      ),
...
test('Mutation.deleteInventoryItem returns boolean from inventoryService.deleteInventoryItem', async () => {
  ...
  const result = await asResolverFn(resolvers.Mutation!.deleteInventoryItem)(
    {},
...
test('Mutation.updateStore passes through GraphQLError unchanged', async () => {
